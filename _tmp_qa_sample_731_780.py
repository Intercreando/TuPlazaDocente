...
lote = data["items"][730:780]
assert len(lote) == 50
rng = random.Random(20260914)
sample = rng.sample(lote, 5)
out = []
for it in sample:
    pos = next(i for i, x in enumerate(lote) if x["id"] == it["id"]) + 731
    out.append(
        {
            "pos": pos,
            "id": it["id"],
...
path = root / "_tmp_qa_sample_731_780.json"
path.write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
print("wave11", data.get("contestUpgradeWave11"))
print("sample ids", [x["id"] for x in out])
src = json.loads((root / "_tmp_lote_731_780.json").read_text(encoding="utf-8"))
ok = 0
