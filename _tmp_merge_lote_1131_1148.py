# -*- coding: utf-8 -*-
"""Parche de questions_v1.json (1131-1148) según reglas_cnsc.md."""
import json
import subprocess
import sys
...
def main():
    r = subprocess.run(
        [sys.executable, str(ROOT / "_tmp_validate_lote_1131_1148.py")],
        cwd=ROOT,
    )
    if r.returncode != 0:
    ...
    lote = data["items"][1130:1148]
    assert len(lote) == 18, len(lote)
    ...
    assert set(upgrades) == set(expected_ids)
    ...
    for src in lote:
        ...
        frozen_meta.append((src["id"], frozen))
        for k in PATCH_KEYS:
            src[k] = dst_item[k]
