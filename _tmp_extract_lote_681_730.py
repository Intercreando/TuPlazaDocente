...
items = data["items"]
lote = items[680:730]  # 1-indexed 681-730
...
for k in data.keys():
    if k != "items":
        print(k, data[k] if not isinstance(data[k], list) else f"list[{len(data[k])}]")
...
with open(root / "_tmp_lote_681_730.json", "w", encoding="utf-8") as f:
