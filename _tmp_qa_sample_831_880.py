...
lote = data["items"][830:880]
assert len(lote) == 50
rng = random.Random(20260914)
sample = rng.sample(lote, 5)
out = []
for it in sample:
    pos = next(i for i, x in enumerate(lote) if x["id"] == it["id"]) + 831
    out.append(
        {
            "pos": pos,
            "id": it["id"],
...
path = root / "_tmp_qa_sample_831_880.json"
path.write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
print("wave13", data.get("contestUpgradeWave13"))
print("sample ids", [x["id"] for x in out])
src = json.loads((root / "_tmp_lote_831_880.json").read_text(encoding="utf-8"))
ok = 0
