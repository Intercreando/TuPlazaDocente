...
items = data["items"]
lote = items[580:630]  # 1-indexed 581-630
...
for i, it in enumerate(lote):
    ...
    print(
        f"{pos}\t{it['id']}\tci={it.get('correctIndex')}\tdiff={it.get('difficulty')}"
        f"\tdifn={it.get('dificultad')}\topt={opts_len}\tda={da_keys}"
...
# Export compact working copies for subagents
out_dir = root
slim = []
for i, it in enumerate(lote):
    slim.append(
...
with open(out_dir / "_tmp_lote_581_630.json", "w", encoding="utf-8") as f:
...
for c in range(5):
    ...
    path = out_dir / f"_tmp_in_{start}_{end}.json"
