...
lote = data["items"][780:830]
assert len(lote) == 50
rng = random.Random(20260914)
sample = rng.sample(lote, 5)
out = []
for it in sample:
    pos = next(i for i, x in enumerate(lote) if x["id"] == it["id"]) + 781
    out.append(
        {
            "pos": pos,
            "id": it["id"],
...
path = root / "_tmp_qa_sample_781_830.json"
path.write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
print("wave12", data.get("contestUpgradeWave12"))
print("sample ids", [x["id"] for x in out])
src = json.loads((root / "_tmp_lote_781_830.json").read_text(encoding="utf-8"))
ok = 0
...
print("frozen ok", ok)
for it in out:
    print("====", it["pos"], it["id"], "ci", it["correctIndex"])
